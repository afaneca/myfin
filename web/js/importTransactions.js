...
const IMPORT_TRX_FIELD_HEADER_VARIATIONS = {
  ...
  AMOUNT: ['amount', 'montante', 'valor', 'montante (eur)', 'montante(eur)', 'montante(€)', 'montante (€)', 'montante( eur )'],
      ...
      .html(`
            <div class="input-field">
                <select class="select-trxs-account" name="accounts">
                    ...
                    ${accsList.map(account => ImportTransactions.renderAccountsSelectOptions(account))
        .join('')}
                </select>
    ...
    $('div#table-wrapper')
      .html(`
             <table id="transactions-table" class="display browser-defaults" style="width:100%">
                ...
                <tbody>
                    ${resp.fillData.map(trx => ImportTransactions.renderStep2TableRow(trx))
        .join('')}
                </tbody>
            </table>
        `);
    ...
    $('#transactions-table tr')
      .each(function (i, row) {
        ...
        let checkBoxSelector = $(this)
          .find('.trx-checkbox-input');
        if (checkBoxSelector && checkBoxSelector[0] && checkBoxSelector[0].checked) {
          let date_timestamp = DateUtils.convertDateToUnixTimestamp($(this)
            .find('.datepicker')
            .val());
          let amount = $(this)
            .find('.trx-amount-input')
            .val();
          let description = $(this)
            .find('.trx-description-textarea')
            .val();
          let entity_id = $(this)
            .find('.entities-select')
            .val();
          ...
          let accountFrom_id = $($($($($(this)
            .find('.accounts-select2')[0])[0].options)[selectedOptionIndex])[0].attributes['val'])
            .val();

          selectedOptionIndex = $($($(this)
            .find('.accounts-select2')[1])[0])[0].selectedIndex;
          let accountTo_id = $($($($($(this)
            .find('.accounts-select2')[1])[0].options)[selectedOptionIndex])[0].attributes['val'])
            .val();
