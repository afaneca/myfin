...
const getAllAccountsForUser = async (req, res, next) => {
    try {
        const sessionData = await CommonsController.checkAuthSessionValidity(req)
        const accsList = await AccountService.getAccountsForUserWithAmounts(
            sessionData.userId,
            false
        )
        res.send(accsList)
...
const deleteAccount = async (req, res, next) => {
    try {
        ...
        if (!AccountService.doesAccountBelongToUser(sessionData.userId, account.account_id)) {
...
export { createAccount, getAllAccountsForUser, deleteAccount, updateAccount }
