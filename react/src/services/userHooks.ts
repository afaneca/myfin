...
import { AccountType, Account } from './authServices';
...
export function useGetTopSummaryValues() {
    ...
    const operatingFundsSum = accounts.reduce((result, current) => {
        if (current.type == AccountType.Checking || current.type == AccountType.Savings
            || current.type == AccountType.Meal || current.type == AccountType.Wallet) {
