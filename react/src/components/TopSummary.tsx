...
import { useGetTopSummaryValues } from '../services/userHooks';
import { formatStringAsCurrency } from '../utils/textUtils';

const TopSummary = () => {
    const { operatingFundsSum, investingSum, debtSum, netWorthsum } = useGetTopSummaryValues();
...
const TopSummaryAmount = (props: TopSummaryLabelValueProps) => {
    ...
    return (
        <Box bgcolor={theme.palette.background.paper} sx={{ borderRadius: 2, display: 'inline-flex', width: 'fit-content' }}>
            <Typography p={1} variant='caption'>{props.value}</Typography>
        </Box>
    )
}
