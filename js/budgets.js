...
export const Budgets = {
  ...
  renderStatusColumn: (budget, type, val, meta) =>
      `<i class="material-icons">${budget.is_open == true
          ? 'lock_open'
          : 'lock'}</i>`,
  renderMonthYearColumn: (budget, type, val, meta) =>
      `<span style="font-weight: bold;">${DateUtils.getMonthsFullName(
          budget.month)}</span></br>
            <span style="font-size: small;">${budget.month}/${budget.year}</span>`,
  renderObservationsColumn: (budget, type, val, meta) =>
      `<span style="cursor: pointer;" data-budget-id="${budget.budget_id}"
          data-budget-is-open="${budget.is_open}"
          class="table-action-icons modal-go-to-details-action">${budget.observations}</span>`,
  renderExpenseColumn: (budget, type, val, meta) =>
      `${StringUtils.formatMoney(budget.debit_amount)}`,
  renderIncomeColumn: (budget, type, val, meta) =>
      `${StringUtils.formatMoney(budget.credit_amount)}`,
  ...
  renderSavingsColumn: (budget, type, val, meta) => {
    return `${Budgets.buildBudgetSavingsRateRow(
        budget.savings_rate_percentage,
        DateUtils.isCurrentMonthAndYear(budget.month, budget.year))}`;
  },
  renderActionsColumn: (budget, type, val, meta) =>
  ...
  buildBudgetBalanceRow: (
      balanceValue, balanceChangePercentage, isCurrentMonth) => {
    let strToReturn = '';
