...
var Rules = {
  ...
  initRulesTable: (rulesArr) => {
    $('#table-wrapper')
      .html(Rules.renderRulesTable(rulesArr));
  ...
  renderRulesTable: (rulesList) => {
    return `
        ...
        <tbody>
        ${rulesList.map(rule => Rules.renderRulesRow(rule))
      .join('')}
        </tbody>
        </table>
        `;
    ...
    return `
            <tr data-id='${rule.rule_id}'>
                ...
                <td>
                    ...
                    ${rule.assign_category_id ? (`'${rule.assign_category_id}'`) : null}, ${rule.assign_entity_id ? (`'${rule.assign_entity_id}'`) : null}, ${rule.assign_type ? (`'${rule.assign_type}'`) : null}, ${rule.assign_is_essential == true ? (`'${rule.assign_is_essential}'`) : null})" class="material-icons table-action-icons">create</i>
  ...
  showEditRuleModal: (ruleID, matcher_description_operator, matcher_description_value, matcher_amount_operator, matcher_amount_value, matcher_type_operator, matcher_type_value,
    ...
    Rules.renderAddRuleForm(accountsList1, entitiesList1, categoriesList1, matcher_description_operator, matcher_description_value, matcher_amount_operator, matcher_amount_value, matcher_type_operator, matcher_type_value, matcher_account_from_id_operator, matcher_account_from_id_value,
      matcher_account_to_id_operator, matcher_account_to_id_value, assign_account_from_id, assign_account_to_id, assign_category_id, assign_entity_id, assign_type, assign_is_essential);
    $('.collapsible')
      .collapsible('open');
    $('#add-rule-btn')
      .text('Atualizar Regra');
    $('#add-rule-collapsible-header')
      .html('<i class=\'material-icons\'>edit</i>Atualizar Regra');
  ...
  editRule: ruleID => {
    ...
    const descriptionOperator = $('select#operator-select-description')
      .val();
    const descriptionValue = $('input#value-input-description')
      .val();
    const amountOperator = $('select#operator-select-amount')
      .val();
    const amountValue = $('input#value-input-amount')
      .val();
    const typeOperator = $('select#operator-select-type')
      .val();
    const typeValue = $('select#value-select-type')
      .val();
    const accountFromIDOperator = $('select#operator-select-account-from')
      .val();

    let accountFromIDValue = $('select#operator-select-account-from-value')
      .val();
    if (accountFromIDValue == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) accountFromIDValue = null;

    const accountToIDOperator = $('select#operator-select-account-to')
      .val();

    let accountToIDValue = $('select#value-select-account-to')
      .val();
    if (accountToIDValue == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) accountToIDValue = null;

    let categoryAssignValue = $('select#value-select-category-assign')
      .val();
    if (categoryAssignValue == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) categoryAssignValue = null;

    let entityAssignValue = $('select#value-select-entity-assign')
      .val();
    if (entityAssignValue == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) entityAssignValue = null;

    let accountFromAssignValue = $('select#value-select-account-from-assign')
      .val();
    ...
    let essentialValue = $('select#value-select-essential-assign')
      .val();
    if (essentialValue == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) essentialValue = null;
  ...
  renderAddRuleForm: (accountsList, entitiesList, categoriesList, matcher_description_operator = null, matcher_description_value = null,
    ...
    assign_account_from_id = null, assign_account_to_id = null, assign_category_id = null, assign_entity_id = null, assign_type = null, assign_is_essential = null) => {

    $('div#add-rule-form-wrapper')
      .html(
        `
             <!---->
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="input-field col s10">
                            <select id="value-select-entity-assign">
                                ...
                                ${entitiesList.map(ent => Rules.renderEntitiesSelectOption(ent.entity_id, ent.name, assign_entity_id))
          .join('')}
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="input-field col s10">
                            <select id="value-select-account-to-assign">
                                ...
                                ${accountsList.map(acc => Rules.renderAccountsSelectOption(acc.account_id, acc.name, assign_account_to_id))
          .join('')}
                            </select>
                        </div>
                    </div>
    ...
    $('select#operator-select-description')
      .formSelect();
    $('select#operator-select-description')
      .on('change', function () {
    ...
    $('select#operator-select-account-from')
      .formSelect();
    $('select#operator-select-account-from-value')
      .select2();
    $('select#operator-select-account-from')
      .on('change', function () {
        if (this.value == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) {
          $('#operator-select-account-from-value')
            .select2('destroy');
          $('#operator-select-account-from-value')
            .prop('disabled', true);
          $('#operator-select-account-from-value')
    ...
    $('select#operator-select-account-to')
      .formSelect();
    $('select#value-select-account-to')
      .select2();
    $('select#operator-select-account-to')
      .on('change', function () {
        if (this.value == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) {
          $('#value-select-account-to')
            .select2('destroy');
          $('#value-select-account-to')
            .prop('disabled', true);
          $('#value-select-account-to')
    ...
    $('select#value-select-category-assign')
      .select2();//.formSelect();
    $('select#value-select-entity-assign')
      .select2();//.formSelect();
    $('select#value-select-account-from-assign')
      .select2();//.formSelect();
    $('select#value-select-account-to-assign')
      .select2();//.formSelect();
    $('select#value-select-essential-assign')
      .select2();//.formSelect();
  },
  addRule: () => {
    const descriptionOperator = $('select#operator-select-description')
    ...
    let accountToAssignValue = $('select#value-select-account-to-assign')
      .val();
    if (accountToAssignValue == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) accountToAssignValue = null;

    let essentialAssignValue = $('select#value-select-essential-assign')
      .val();
    if (essentialAssignValue == MYFIN.RULES_OPERATOR.DEFAULT_RULES_OPERATOR_IGNORE) accountToAssignValue = null;
