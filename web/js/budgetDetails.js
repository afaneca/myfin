...
var BudgetDetails = {
  init: (isOpen, isNew, budgetID) => {
    ...
    if (isOpen == true) {
      $('#conclusion-close-btn')
        .show();
      $('#conclusion-close-btn-text')
        .text('Fechar Orçamento');
  ...
  setupBudgetInputs: (selectorID, categoriesArr, isCredit, sameMonthLastYearLabel) => {
    $(selectorID)
      .html(BudgetDetails.buildBudgetInputs(categoriesArr, isCredit, sameMonthLastYearLabel));
    ...
    $('#table_total_credit_current')
      .text(StringUtils.formatMoney(incomeAcc));
    $('#table_total_debit_current')
  ...
  buildBudgetInputs: (categoriesArr, isCredit, sameMonthLastYearLabel) => {
    return `
      ...
      .map(cat => BudgetDetails.renderInputRow(cat, isCredit, sameMonthLastYearLabel))
      .join('')}
  ...
  onCategoryTooltipClick: (catID, isCredit) => {
    let datepickerValue = $('#budgets-monthpicker')
      .val();
  ...
  setInitialBalance: value => {
    $('span#estimated_initial_balance_value')
      .text(StringUtils.formatMoney(value));
  },
  setObservations: value => {
    $('textarea#budget_observations')
      .text(value);
  },
  onConclusionClicked: () => {
    let observations = StringUtils.removeLineBreaksFromString($('#budget_observations')
      .val());
    let datepickerValue = $('#budgets-monthpicker')
      .val();
    let month = parseInt(datepickerValue.substring(0, 2));
