...
class NewRelicHandlerTest extends TestCase
{
    ...
    public static $transaction_name;
    ...
    public function testTheTransaction_nameIsNullByDefault()
    {
        $handler = new StubNewRelicHandler();
        $handler->handle($this->getRecord(Logger::ERROR, 'log message'));

        $this->assertEquals(null, self::$transaction_name);
    }

    public function testTheTransaction_nameCanBeInjectedFromTheConstructor()
    {
        $handler = new StubNewRelicHandler(Logger::DEBUG, false, null, false, 'myTransaction');
        $handler->handle($this->getRecord(Logger::ERROR, 'log message'));

        $this->assertEquals('myTransaction', self::$transaction_name);
    }

    public function testTheTransaction_nameCanBeOverriddenFromEachLog()
    {
        $handler = new StubNewRelicHandler(Logger::DEBUG, false, null, false, 'myTransaction');
        $handler->handle($this->getRecord(Logger::ERROR, 'log message', array('transaction_name' => 'logTransactName')));

        $this->assertEquals('logTransactName', self::$transaction_name);
    }
}
...
function newrelic_name_transaction($transaction_name)
{
    return NewRelicHandlerTest::$transaction_name = $transaction_name;
}
