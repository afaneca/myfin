...
const createTransactionStep0 = async (userId: bigint, dbClient = undefined) => {
  ...
  const entities = await EntityService.getAllEntitiesForUser(userId, dbClient);
  const categories = await CategoryService.getAllCategoriesForUser(userId, dbClient);
  const accounts = await AccountService.getActiveAccountsForUser(userId, dbClient);
...
const createTransaction = async (
  ...
  switch (trx.type) {
    case MYFIN.TRX_TYPES.INCOME:
      ...
      await AccountService.setNewAccountBalance(userId, trx.account_to_id, newBalance, dbClient);
      break;
    case MYFIN.TRX_TYPES.EXPENSE:
      ...
      await AccountService.setNewAccountBalance(userId, trx.account_from_id, newBalance, dbClient);
      break;
...
const deleteTransaction = async (userId: bigint, transactionId: number, dbClient = undefined) => {
  ...
  const trx = await dbClient.transactions
    .findUniqueOrThrow({
      where: {
  ...
  const accountsCount = await dbClient.accounts.count({
    where: {
  ...
  // Delete transaction
  await dbClient.transactions.delete({
    where: {
      transaction_id: transactionId,
    },
  });
  ...
  switch (oldType) {
    case MYFIN.TRX_TYPES.INCOME:
      ...
      await AccountService.setNewAccountBalance(userId, oldAccountTo, newBalance, dbClient);
      break;
    case MYFIN.TRX_TYPES.EXPENSE:
      newBalance = await AccountService.recalculateBalanceForAccountIncrementally(
        oldAccountFrom,
      ...
      await AccountService.setNewAccountBalance(userId, oldAccountFrom, newBalance, dbClient);
      break;
    case MYFIN.TRX_TYPES.TRANSFER:
    default:
      newBalance = await AccountService.recalculateBalanceForAccountIncrementally(
...
const updateTransaction = async (
  ...
  dbClient = await getPrismaTransactionInstance(dbClient);
  const outdatedTrx = await dbClient.transactions.findUniqueOrThrow({
  ...
  await dbClient.transactions.update({
    where: { transaction_id: trx.transaction_id },
    data: {
  ...
  switch (oldType) {
    case MYFIN.TRX_TYPES.INCOME:
      await AccountService.changeBalance(userId, oldAccountTo, -oldAmount, dbClient);
      ...
      break;
    case MYFIN.TRX_TYPES.EXPENSE:
      await AccountService.changeBalance(userId, oldAccountFrom, -oldAmount, dbClient);
      await AccountService.recalculateBalanceForAccountIncrementally(
        oldAccountFrom,
        oldTimestamp - 1,
        DateTimeUtils.getCurrentUnixTimestamp() + 1,
